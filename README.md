...
flutterfire configure --project=like-him-app --platforms=android,ios,web --yes
```
...
works when called from a build of `dev.airborneeagle.likehim` signed with the
upload keystore's cert (SHA-1 `EE:2B:F8:CA:07:93:8D:89:52:6C:49:26:37:E3:C6:40:22:F3:32:53`),
iOS key only works for that bundle id, web key only from `like-him-app.web.app` /
`like-him-app.firebaseapp.com` / localhost. So even if a config file leaks,
...
firebase deploy --only firestore:rules --project=like-him-app

# Re-deploy web build to Firebase Hosting
flutter build web --release && firebase deploy --only hosting --project=like-him-app

# Re-run flutterfire configure (after bundle ID changes)
flutterfire configure --project=like-him-app
```
...
2. Create the app. Suggested settings:
   - Package name: `dev.airborneeagle.likehim` (matches what's in
...
6. Privacy policy URL: <https://like-him-app.web.app/privacy>
...
├── data/
│   └── attributes_data.dart  all 10 attributes + every statement, verbatim
